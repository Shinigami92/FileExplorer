...
namespace FileExplorer
{
	...
	public sealed partial class MainPage : Page
	{
		...
		{
			this.MenuSplitViewMainLeft.IsPaneOpen = !this.MenuSplitViewMainLeft.IsPaneOpen;
		}
		...
		private async void MenuButtonMainAddFolder_ItemClick(object sender, ItemClickEventArgs e)
		{
			...
			this.MenuButtonMainAddFolder.SelectedIndex = -1;
		}
		...
		private void MenuListViewItemRemove_Click(object sender, RoutedEventArgs e)
		{
			...
			var f = this.MenuFolderItems.ToList().Find(item => item.Folder == storageFolder);
		...
		private async void UpdateCurrentFolderPathPanel()
		{
			if (this.currentFolder != null)
			{
				this.CurrentFolderPathPanel.Children.Clear();
				...
				parts.Reverse();
				this.CurrentFolderPathPanel.Children.Add(BuildCurrentFolderPathButton(parts.ElementAt(0)));
				for (var i = 1; i < parts.Count; i++)
				{
					this.CurrentFolderPathPanel.Children.Add(BuildCurrentFolderPathSeperator());
					this.CurrentFolderPathPanel.Children.Add(BuildCurrentFolderPathButton(parts.ElementAt(i)));
				}
			}
		}
		...
		private Button BuildCurrentFolderPathButton(StorageFolder folder)
		{
			...
			btn.Click += this.NavigateTo_Click;
			return btn;
		}
		...
		{
			this.FolderUpButton.LabelPosition = CommandBarLabelPosition.Default;
		}

		private void CommandBar_Closing(object sender, object e)
		{
			this.FolderUpButton.LabelPosition = CommandBarLabelPosition.Collapsed;
		}
		...
		private void MenuSplitViewMainLeft_SizeChanged(object sender, SizeChangedEventArgs e)
		{
			...
			{
				this.MenuSplitViewMainLeft.DisplayMode = SplitViewDisplayMode.CompactOverlay;
			}
			else
			{
				this.MenuSplitViewMainLeft.DisplayMode = SplitViewDisplayMode.CompactInline;
			}
		}
		...
		{
			if (this.FileItemsListView.Visibility == Visibility.Visible)
			{
				this.ToggleViewButton.Icon = new SymbolIcon(Symbol.List);
				this.FileItemsListView.Visibility = Visibility.Collapsed;
				this.FileItemsGridView.Visibility = Visibility.Visible;
			}
			else
			{
				this.ToggleViewButton.Icon = new SymbolIcon(Symbol.ViewAll);
				this.FileItemsListView.Visibility = Visibility.Visible;
				this.FileItemsGridView.Visibility = Visibility.Collapsed;
			}
		}
